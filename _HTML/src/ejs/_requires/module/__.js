...
function wrapper (app) {
	...
	/**
	 * Создание контента для js файлов с переводоми скриптовых плагинов
	 *
	 * @param {string} [jsLang="ru"] - язык переводов
	 * @returns {string} js контент с объектом переводов
	 * @memberOf app.module
	 * @tutorial app.module.__
	 * @sourceCode
	 */
	__.jsTranslations = function (jsLang = 'ru') {
		const translates = {};
		const data = [
			lodash.cloneDeep(app.require('module/l18n/magnific-popup.json')),
			lodash.cloneDeep(app.require('module/l18n/jquery-validation.json')),
			lodash.cloneDeep(app.require('module/l18n/select2.json'))
		];
		...
		data.forEach(group => {
			for (let key in group) {
				...
				translates[g][k] = group[key][jsLang];
			}
		});

		return [
			';(function(window) {',
			`window.jsTranslations = ${JSON.stringify(translates, null, '\t')};`,
			'})(window);'
		].join('\n');
	};
	...
	__.translations = {
		'general': lodash.merge({},
			app.require('./l18n/general.json', __dirname)
		),
		'js-plugins': lodash.merge({},
			app.require('./l18n/jquery-validation.json', __dirname),
			app.require('./l18n/magnific-popup.json', __dirname),
			app.require('./l18n/select2.json', __dirname)
		)
	};
