...
const {task, argv, paths, ftpPresets, watchers, WezomCMS, useCritical, styleLoaderInsertAtId} = require('./config.html');
...
const gulpTasks = {
		...
		const options = {
			ejs: {
				...
				afterRender (markup, x, sources) {
					...
					if (argv.production && !/js-translations/.test(filePath)) {
	...
	'sass-main' () {
		...
		const taskPaths = {
			src: [
				'./_HTML/src/sass/vendors-and-helpers.scss',
				'./_HTML/src/sass/style.scss',
				'./_HTML/src/sass/editor.scss',
				'./_HTML/src/sass/noscript.scss'
			],
	...
	'sass-critical' () {
		if (useCritical !== true) {
			return gulp.task('sass-critical', (done) => {
				logger('red', 'Critical task disabled! See config.html => configSystem.useCritical');
				done();
			});
		}
		...
		const taskPaths = {
			src: [
				...
				'./_HTML/src/sass/critical/generated/*.scss'
			],
			dest: path.join(paths.assets, '/css/critical/'),
			base: './_HTML/src/sass/critical/'
		};
