...
function coreExtend (app) {
	...
	if (app.data.argv.ftp) {
		app.data.usePHP = false;
	}
	...
	/**
	 * href атрибут для ссылки на главную страницу
	 * @type {string}
	 * @memberOf app.data
	 * @sourceCode
	 */
	app.data.indexHref = app.data.currentView.href === app.data.views.index.href ? '' : ` href="${app.data.views.index.href}"`;

	app.el.El = app.require('el/El.js')(app);
	app.el.ElSVG = app.require('el/ElSVG.js')(app);
	app.el.SvgIcon = app.require('el/SvgIcon.js')(app);
	app.el.Button = app.require('el/Button.js')(app);
	app.el.FormInput = app.require('el/FormInput.js')(app);
	app.el.FormCheck = app.require('el/FormCheck.js')(app);
	app.el.FormTextarea = app.require('el/FormTextarea.js')(app);
	app.el.FormSelect = app.require('el/FormSelect.js')(app);
	app.el.LabelError = app.require('el/LabelError.js')(app);
}
